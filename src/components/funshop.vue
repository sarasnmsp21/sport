<template>
  <div class="funshop">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12 mx-auto p-0 relative">
          <div class="pt-70">
            <div class="font-oswald heading-fontsize">
              <span class="color-lightblack">FUN</span
              ><span class="color-yellow"> SHOP</span>
            </div>
            <div
              class="col-md-8 mx-auto color-lightgray fun-parafontsize mt-4 mb-4"
            >
              It is a long established fact that a reader will be distracted by
              the readable content of a page when looking at its layout. as
              opposed to using 'Content here, content here', making it look like
              readable English.
            </div>
            <div>
              <span class="bg-lightblack work-sans save-font">
                <span class="color-white">SAVE</span>
                <span class="color-yellow"> 33% OFF</span>
                <span class="color-white"> for all new orders</span>
              </span>
            </div>
            <div class="mt-5 pb-5">
              <md-card>
                <md-card-media>
                  <!-- swiper -->
                  <swiper :options="swiperOption">
                    <swiper-slide>
                      <div>
                        <img
                          src="../assets/images/shoe1.jpeg"
                          alt=""
                          width="200px"
                        />
                        <div class="d-flex">
                          <div
                            class="font-oswald color-lightblack fun-parafontsize bg-lightblack flex-fill p-2"
                          >
                            <span class="color-white mr-1 line-through"
                              >$190</span
                            >
                            <span class="color-yellow">$250</span>
                          </div>
                          <div
                            class="font-oswald color-lightblack  bg-orange buy-fontsize  flex-fill p-2"
                          >
                            BUY
                          </div>
                        </div>
                      </div>
                    </swiper-slide>
                    <swiper-slide>
                      <div>
                        <img
                          src="../assets/images/shoe1.jpeg"
                          alt=""
                          width="200px"
                        />
                        <div class="d-flex">
                          <div
                            class="font-oswald color-lightblack fun-parafontsize bg-lightblack flex-fill p-2"
                          >
                            <span class="color-white mr-1 line-through"
                              >$190</span
                            >
                            <span class="color-yellow">$250</span>
                          </div>
                          <div
                            class="font-oswald color-lightblack  bg-orange buy-fontsize  flex-fill p-2"
                          >
                            BUY
                          </div>
                        </div>
                      </div>
                    </swiper-slide>
                    <swiper-slide>
                      <div>
                        <img
                          src="../assets/images/shoe1.jpeg"
                          alt=""
                          width="200px"
                        />
                        <div class="d-flex">
                          <div
                            class="font-oswald color-lightblack fun-parafontsize bg-lightblack flex-fill p-2"
                          >
                            <span class="color-white mr-1 line-through"
                              >$190</span
                            >
                            <span class="color-yellow">$250</span>
                          </div>
                          <div
                            class="font-oswald color-lightblack  bg-orange buy-fontsize  flex-fill p-2"
                          >
                            BUY
                          </div>
                        </div>
                      </div>
                    </swiper-slide>
                    <swiper-slide>
                      <div>
                        <img
                          src="../assets/images/shoe1.jpeg"
                          alt=""
                          width="200px"
                        />
                        <div class="d-flex">
                          <div
                            class="font-oswald color-lightblack fun-parafontsize bg-lightblack flex-fill p-2"
                          >
                            <span class="color-white mr-1 line-through"
                              >$190</span
                            >
                            <span class="color-yellow">$250</span>
                          </div>
                          <div
                            class="font-oswald color-lightblack  bg-orange buy-fontsize  flex-fill p-2"
                          >
                            BUY
                          </div>
                        </div>
                      </div>
                    </swiper-slide>
                    <swiper-slide>
                      <div>
                        <img
                          src="../assets/images/shoe1.jpeg"
                          alt=""
                          width="200px"
                        />
                        <div class="d-flex">
                          <div
                            class="font-oswald color-lightblack fun-parafontsize bg-lightblack flex-fill p-2"
                          >
                            <span class="color-white mr-1 line-through"
                              >$190</span
                            >
                            <span class="color-yellow">$250</span>
                          </div>
                          <div
                            class="font-oswald color-lightblack  bg-orange buy-fontsize  flex-fill p-2"
                          >
                            BUY
                          </div>
                        </div>
                      </div>
                    </swiper-slide>
                    <swiper-slide>
                      <div>
                        <img
                          src="../assets/images/shoe1.jpeg"
                          alt=""
                          width="200px"
                        />
                        <div class="d-flex">
                          <div
                            class="font-oswald color-lightblack fun-parafontsize bg-lightblack flex-fill p-2"
                          >
                            <span class="color-white mr-1 line-through"
                              >$190</span
                            >
                            <span class="color-yellow">$250</span>
                          </div>
                          <div
                            class="font-oswald color-lightblack  bg-orange buy-fontsize  flex-fill p-2"
                          >
                            BUY
                          </div>
                        </div>
                      </div></swiper-slide
                    >
                    <swiper-slide>
                      <div>
                        <img
                          src="../assets/images/shoe1.jpeg"
                          alt=""
                          width="200px"
                        />
                        <div class="d-flex">
                          <div
                            class="font-oswald color-lightblack fun-parafontsize bg-lightblack flex-fill p-2"
                          >
                            <span class="color-white mr-1 line-through"
                              >$190</span
                            >
                            <span class="color-yellow">$250</span>
                          </div>
                          <div
                            class="font-oswald color-lightblack  bg-orange buy-fontsize  flex-fill p-2"
                          >
                            BUY
                          </div>
                        </div>
                      </div></swiper-slide
                    >
                    <swiper-slide>
                      <div>
                        <img
                          src="../assets/images/shoe1.jpeg"
                          alt=""
                          width="200px"
                        />
                        <div class="d-flex">
                          <div
                            class="font-oswald color-lightblack fun-parafontsize bg-lightblack flex-fill p-2"
                          >
                            <span class="color-white mr-1 line-through"
                              >$190</span
                            >
                            <span class="color-yellow">$250</span>
                          </div>
                          <div
                            class="font-oswald color-lightblack  bg-orange buy-fontsize  flex-fill p-2"
                          >
                            BUY
                          </div>
                        </div>
                      </div></swiper-slide
                    >
                    <swiper-slide>
                      <div>
                        <img
                          src="../assets/images/shoe1.jpeg"
                          alt=""
                          width="200px"
                        />
                        <div class="d-flex">
                          <div
                            class="font-oswald color-lightblack fun-parafontsize bg-lightblack flex-fill p-2"
                          >
                            <span class="color-white mr-1 line-through"
                              >$190</span
                            >
                            <span class="color-yellow">$250</span>
                          </div>
                          <div
                            class="font-oswald color-lightblack  bg-orange buy-fontsize  flex-fill p-2"
                          >
                            BUY
                          </div>
                        </div>
                      </div></swiper-slide
                    >
                    <swiper-slide>
                      <div>
                        <img
                          src="../assets/images/shoe1.jpeg"
                          alt=""
                          width="200px"
                        />
                        <div class="d-flex">
                          <div
                            class="font-oswald color-lightblack fun-parafontsize bg-lightblack flex-fill p-2"
                          >
                            <span class="color-white mr-1 line-through"
                              >$190</span
                            >
                            <span class="color-yellow">$250</span>
                          </div>
                          <div
                            class="font-oswald color-lightblack  bg-orange buy-fontsize  flex-fill p-2"
                          >
                            <span>BUY</span>
                          </div>
                        </div>
                      </div></swiper-slide
                    >
                  </swiper>
                </md-card-media>
              </md-card>
              <div class="mt-4">
                <span class="btn-border work-sans">
                  <span class="bg-orange btn-padding">VIEW ALL</span>
                </span>
              </div>
            </div>

            <div class="banner-img"></div>
          </div>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
export default {
  data() {
    return {
      swiperOption: {
        slidesPerView: 1,
        spaceBetween: 22,
        centeredSlides: true,
        pagination: {
          el: ".swiper-pagination",
          clickable: true
        }
      }
    };
  }
};
</script>
<style lang="scss">
@import "../assets/scss/variables.scss";
.heading-fontsize {
  font-size: 30px;
}
.fun-parafontsize {
  font-size: 14px;
}
.save-font {
  padding: 10px 11px;
  font-size: 25px;
}
.swiper-slide {
  width: 200px !important;
}
.banner-img {
  background: url(../assets/images/fun.jpg);
  height: 358px;
  position: absolute;
  z-index: -1;
  width: 100%;
  top: 265px;
}
.clear {
  clear: both;
}
.line-through {
  text-decoration: line-through;
}
.buy-fontsize {
  font-size: 10px;
}
</style>
